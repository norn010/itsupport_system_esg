...
-- SAFETY: All commands use 'IF OBJECT_ID ... IS NULL' or 'IF COL_LENGTH ... IS NULL'.
-- =========================================================================
...
-----------------------------------------------------------
-- 2. CATEGORIES & SUBCATEGORIES (Core for Tickets/ITAM)
-----------------------------------------------------------
...
BEGIN
    CREATE TABLE tickets (
        ...
        closed_at DATETIME,
        asset_id INT NULL -- Linked to Asset
    );
END;

-- Safety checks to add columns if they weren't in initial create
...
IF COL_LENGTH('dbo.tickets', 'resolved_at') IS NULL ALTER TABLE tickets ADD resolved_at DATETIME NULL;
...
BEGIN
    CREATE TABLE ticket_internal_notes (
        id INT IDENTITY(1,1) PRIMARY KEY,
        ticket_id NVARCHAR(50) NOT NULL, -- Logical ID
...
-----------------------------------------------------------
-- 5. ITAM MODULE (Asset Management)
-----------------------------------------------------------
...
-----------------------------------------------------------
-- 6. TICKET ACTIVITY LOG & METADATA
-----------------------------------------------------------
...
-- DONE
PRINT 'Database complete setup script finished successfully.';
